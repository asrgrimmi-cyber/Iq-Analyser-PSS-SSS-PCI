...
static void kf_bfly3(kiss_fft_cpx * Fout, const size_t fstride, const kiss_fft_cfg st, size_t m) {
     ...
     do {
         ...
         Fout[m].r = Fout->r - 0.5f * scratch[3].r;
         Fout[m].i = Fout->i - 0.5f * scratch[3].i;
...
static void kf_work(kiss_fft_cpx * Fout, const kiss_fft_cpx * f, const size_t fstride, int in_stride, int * factors, const kiss_fft_cfg st) {
    ...
    if (m == 1) {
        ...
        do { kf_work(Fout, f, fstride * p, in_stride, factors, st); f += fstride * in_stride; } while ((Fout += m) != Fout_end);
    }
    Fout -= p * m;
    switch (p) {
...
kiss_fft_cfg kiss_fft_alloc(int nfft, int inverse_fft, void * mem, size_t * lenmem) {
    ...
    if (st) {
        int i;
        st->nfft = nfft; st->inverse = inverse_fft;
        for (i = 0; i < nfft; ++i) {
